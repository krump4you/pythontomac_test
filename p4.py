...
def split_file(root):
    ...
    for k, row in enumerate(rows):
        def long_running_task():
            ...
            if progress % 2 == 0:
                if progress % 4 == 0:
                    ...
                    root.after(1000, clear_label)  # Clear label after 1 second
        ...
        if is_valid == "skip":
            print(f"Ошибка проверки номера в строке №:{(k-1)}...Пропускаем...")
            continue
...
def main():
    ...
    split_button = tk.Button(root, text="Фильтровать", command=lambda: split_file(root))
